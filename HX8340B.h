...
class HX8340B : public Adafruit_GFX
{
	...
	public:
		...
		void begin(), setWindow(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1),
				fillScreen(uint16_t c), pushColor(uint16_t c), drawPixel(int16_t x,
						int16_t y, uint16_t color), drawFastVLine(int16_t x,
						int16_t y, int16_t h, uint16_t color), drawFastHLine(
						int16_t x, int16_t y, int16_t w, uint16_t color), fillRect(
						int16_t x, int16_t y, int16_t w, int16_t h, uint16_t c),
				invertDisplay(uint8_t i);
		uint16_t Color565(uint8_t r, uint8_t g, uint8_t b);

		// Consider making these private:
		void writeCommand(uint8_t c), writeData(uint8_t c);
