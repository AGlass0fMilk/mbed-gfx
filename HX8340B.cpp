...
void HX8340B::writeData(uint8_t c) {
  // Prepend leading bit instead of using D/C pin
  if (hwSPI) {
    SPCR        = 0;            // Disable SPI temporarily
...
void HX8340B::setWindow(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1)
{
  uint8_t t0, t1;
  *csport &= ~cspinmask;
...
// clear everything
void HX8340B::fillScreen(uint16_t c) {
...
void HX8340B::pushColor(uint16_t color) {
  *csport &= ~cspinmask;
...
void HX8340B::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void HX8340B::drawFastVLine(int16_t x, int16_t y, int16_t h,
 uint16_t color) {
...
void HX8340B::drawFastHLine(int16_t x, int16_t y, int16_t w,
  uint16_t color) {
...
void HX8340B::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) {
...
uint16_t HX8340B::Color565(uint8_t r, uint8_t g, uint8_t b) {
  return ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >> 3);
}

void HX8340B::invertDisplay(uint8_t i) {
}
